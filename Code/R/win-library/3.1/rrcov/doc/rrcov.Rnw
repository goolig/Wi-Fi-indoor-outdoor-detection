...
the data set \code{diabetes} from package \pkg{mclust}: the grouping
variable is \code{diabetes$class} and all remaining variables are the
explanatory variables. The code
\begin{Scode}
R> x <- diabetes[,-1]
